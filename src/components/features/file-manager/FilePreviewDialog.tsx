...
export function FilePreviewDialog({
  ...
  const [previewName, setPreviewName] = useState(file?.encryptedName || '')
  ...
  useEffect(() => {
    ...
    if (file) {
      setPreviewName(file.encryptedName)
    }
  }, [open, file])
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText(`https://uoozer.app/s/${file.id}`)
    setCopied(true)
    setTimeout(() => setCopied(false), 2000)
  }

  const handleRenameSubmit = () => {
    if (previewName.trim() === file.encryptedName || isSaving) {
      setIsEditing(false)
      return
    }
    setIsSaving(true)
    setTimeout(() => {
      if (onRename) onRename(file.id, false, previewName)
      setIsSaving(false)
      setIsEditing(false)
    }, 600)
  }
              ...
              >
                ...
                <div className="prose prose-sm max-w-none space-y-4 leading-relaxed text-neutral-600">
                  <p>
                    ...
                    production zero-knowledge environment, the actual file content would be fetched
                    in encrypted chunks from the server, decrypted client-side using your Master
                    Key, and rendered securely here in the DOM.
                  </p>
                </div>
              </div>
            )}
