...
export function useItemActions(
  ...
  const { deleteItemAsync, renameItem, createFolder, restoreItemAsync, permanentDeleteItemAsync } =
    useVaultActions()
  ...
  const handleDelete = async (): Promise<void> => {
    ...
    await deleteItemAsync({ id: item.id, isFolder })
  }
  const isTrash = !!item?.deletedAt
  const handleRestore = async (): Promise<void> => {
    if (!item || isShareMode) return
    await restoreItemAsync({ id: item.id, isFolder })
  }
  const handlePermanentDelete = async (): Promise<void> => {
    if (!item || isShareMode) return
    await permanentDeleteItemAsync({ id: item.id, isFolder })
  }
      ...
      if (isFolder) {
        ...
        const payloadJson = new TextEncoder().encode(
          JSON.stringify({
            ...
            name: item.name,
            mime_type: (item as FileItem).mimeType || 'application/octet-stream',
          })
        )
        const encryptedPayload = await encryptMetadata(payloadJson, shareKey)
        payload = {
          item_type: 'file',
          encrypted_payload: await bytesToBase64(encryptedPayload.ciphertext),
          encrypted_nonce: await bytesToBase64(encryptedPayload.nonce),
